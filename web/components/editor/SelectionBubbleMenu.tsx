...
export function SelectionBubbleMenu({ editor }: { editor: Editor }) {
  ...
  return (
    <BubbleMenu
      ...
      className="glass-strong flex items-center gap-0.5 rounded-xl px-1 py-1 shadow-glass-lg"
    >
      ...
      <span className="mx-0.5 h-5 w-px bg-slate-400/50" />
...
function Mark({
  ...
  return (
    <button
      ...
      onClick={onClick}
      aria-pressed={active}
      className={`flex h-7 min-w-7 items-center justify-center rounded-lg px-1.5 text-sm font-semibold transition
        ${active ? 'bg-indigo-500 text-white' : 'text-slate-800 hover:bg-white/70 hover:text-slate-900'} ${className}`}
    >
      {label}
    </button>
  );
